...
@Component({
  selector: 'app-comment-queue-page',
  templateUrl: './comment-queue-page.component.html',
  styleUrls: ['./comment-queue-page.component.css']
})
export class CommentQueuePageComponent implements OnInit {
 ...
 */
  ngOnInit(): void {
    ...
    this._appSettingsService.getCommentAutoApprove().subscribe(response => {
      this.commentsAutoApprove = response['autoApprove'];
      if (this.commentsAutoApprove) {
        this.status = this.statusOptions[0]['name'];
      }
    })
 ...
 */
  getComments(res) {
    ...
    if (res != null) {
      ...
      this.returnedComments = returnedCommentsOG.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
 ...
 */
  async deleteComment(index: number) {
    ...
    title: String
    var comment = Object.assign({}, this.returnedComments[index]);
    await new Promise((resolve, reject) => {
      this._mediaService.getMedia(mediaID).subscribe(media => {
        this.logChanges(media.title, change, mediaID, comment)
        resolve();
      })
    })
 ...
 */
  ...
  rejectComment(comment: Comment, mediaItem: Media) {
    this.rejectConfirmModal(comment, mediaItem).subscribe(response => {
      console.log(response);
    });
  }

  rejectConfirmModal(comment: Comment, mediaItem: Media) {
    const dialogConfig = new MatDialogConfig();
    dialogConfig.data = { comment: comment, mediaItem: mediaItem }
    dialogConfig.minWidth = 600;

    const dialogRef = this._matDialog.open(RejectCommentModalComponent, dialogConfig);
